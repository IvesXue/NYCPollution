...
data <- read.csv("hosprate-by-modzcta.csv")

# Step 2: Convert the data from wide to long format
data_long <- pivot_longer(data, cols = -date, names_to = "Location", values_to = "HospRate")
...
data_long <- data_long %>%
  mutate(ZipCode = str_replace(Location, "HOSPRATE_", "")) %>%
  select(-Location) # Remove the 'Location' column

# Step 3: Reorder columns to match the desired output
data_long <- select(data_long, Year, Month, HospRate, ZipCode)

# Export the transformed data to a new CSV file
write.csv(data_long, "transformed_dataset_hosperate.csv", row.names = FALSE)
